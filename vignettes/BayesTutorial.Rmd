...
```{r}
dat<-simTraitMatch(size_x = 8,size_y = 9,traits_x = rpois(8,7),traits_y = rpois(9,10),alpha_sigma = 3,replicates=5)
```
...
```{r}
intercept<-fitBayesreg(dat = dat,algorithm = "Intercept",draws=50000)
```
...
```{r}
predframe<-predictBayesreg(intercept)
...
```{r}
binomial<-fitBayesreg(dat=dat,algorithm = "Binomial",draws=50000)
```

Just as before, we can get the predicted matrix based on the model fit.

```{r}
predframe2<-predictBayesreg(binomial)
...
```{r}
multinomial<-fitBayesreg(dat,algorithm = "Multinomial")
```
...
```{r}
#extract pars
predframe3<-predictBayesreg(multinomial)
...
```{r}
intercept_stats<-networkBayesReg(predframe,indices=c("nestedness","connectance"))
intercept_stats$Model<-"Intercept"
binomial_stats<-networkBayesReg(predframe2,indices=c("nestedness","connectance"))
binomial_stats$Model<-"Binomial"

allstats<-bind_rows(list(intercept_stats,binomial_stats))

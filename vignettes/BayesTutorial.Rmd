...
```{r}
intercept<-fit.bayesreg(dat = dat,algorithm = "Intercept",draws=1000)
```
...
```{r}
binomial<-fit.bayesreg(dat=dat,algorithm = "Binomial",draws=1000)
```
...
```{r}
sumframe3<-predframe3 %>% group_by(I,J) %>%  summarize(mean=mean(estimate),lower=quantile(estimate,0.05),upper=quantile(estimate,0.95)) 
...
ggplot(data=justB,aes(x=J,y=estimate)) + geom_boxplot(fill="black") + labs(y="Probability of interaction") + ggtitle("For Species B in the higher level") + theme_bw()
```
...
allsums<-bind_rows(list(sumframe1,sumframe2,sumframe3))
```

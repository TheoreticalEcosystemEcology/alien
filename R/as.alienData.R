...
#' and the species identifier (\code{idSp}) on the second. There are two
...
as.alienData <- function(idObs, interactPair = NULL, coOcc = NULL, coAbund = NULL,
    ...
    # if (ncol(idObs) < 2) {
    #     stop("idObs must be a matrix/dataframe with at leats 2 columns")
    # } else {
    ...
    # }
    # head(idObs) %T>% print
    # # colnames(idObs)[1:2] <- c('idSite', 'idSp') if (ncol(idObs) > 2) { if
    # # (ncol(idObs) == 4) { colnames(idObs)[3:4] <- c('idTime', 'idInd') } else
    # # idObs$idInd <- NA_character } else idObs$idInd <- idObs$idTime <- NA_character
    # # if (verbose) message(''idObs' columns' names are
    # # 'idSite','idSp','idTime','idInd'') }

...
#' @details
#' 
#' The strength of the interactions defined in the third column of \code{interactPair} can be an NA or any numerical value. In addition, species location (sites), will be converted to a factor. Note that there can be multiple sets of locations that can be considered, each one defined by a different factor. 
#' 
#' @return
#'
...
#' @export
as.alienData <-
function(interactPair=NULL,coOcc=NULL,coAbund=NULL,interact=NULL,siteSp=NULL,siteEnv=NULL,traitSp=NULL,traitInd=NULL,phylo=NULL,resCon=NULL,location=NULL,scaleSiteEnv=TRUE,scaleTrait=TRUE,interceptSiteEnv=TRUE,interceptTrait=TRUE){
#### F. Guillaume Blanchet - December 2016
##########################################################################################
	...
	if(!is.null(interactPair)){
		...
		if(is.null(colnames(interactPair))){
			...
			colnames(interactPair)[4:ncol(interactPair)]<-paste("site",1:(ncol(interactPair)-3),sep="")
	...
	if(!is.null(traitInd)){
		...
		if(is.null(column(traitInd))){
			...
			colnames(traitInd)[2:ncol(traitInd)]<-paste("trait",1:(ncol(traitInd)-1),sep="")
	...
	#===============================
	### resCon is a special case that needs to be checked independently	
	if(!is.null(resCon)){
		### Check format
		...
		interact[(nrow(resCon)+1):(ncol(resCon)+nrow(resCon)),(ncol(resCon)+1):(nrow(resCon)+ncol(resCon))]<-t(resCon)
	}
	...
	#### Return results
	#==================
	## Test which obj exists
	args <- c("coOcc","coAbund","interact","siteSp","siteEnv","traitSp","traitInd","phylo","resCon","location")
	coOcc=NULL,coAbund=NULL,interact=NULL,siteSp=NULL,siteEnv=NULL,traitSp=NULL,traitInd=NULL,phylo=NULL,resCon=NULL,location=NULL
	
	existArgs <- sapply(args,exists)
	existArgs <- names(existArgs[existArgs==TRUE])

	## Create res list with NULL
	res <- list(coOcc=NULL,coAbund=NULL,interact=NULL,siteSp=NULL,siteEnv=NULL,traitSp=NULL,traitInd=NULL,phylo=NULL,resCon=NULL,location=NULL)

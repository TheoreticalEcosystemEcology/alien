#' @name fitMC
#'
#' @title Matching centrality as described in Rohr 2014 and 2016.
#'
#' @description Fit matching centrality models among species or individus
#'
...
fitMC <- function(data, d = 1, mxt = 10) {
    ...
    # tidyParamMC(tmp$) estimateMC(tmp$)
    out <- NULL
    # list(as.alienPredNet(network) + qq metric decrit poison + latent traits
    out
}
...
tidyParamMC <- function(nset1, nset2, B1, B2, d = 1, ...) {
    ...
    ## get c1 and c2
    tmp$c1 <- unlist(args[k + (1:(nset1 - 1))])
    tmp$c2 <- unlist(args[k + nset1 - 1 + (1:(nset2 - 1))])
...
estimateMC <- function(netObs, M1, M2, c1, c2, Lambda, delta1, delta2, m) {
    out <- netObs * 0
    cent1 <- c1 * delta1
    cent2 <- c2 * delta2
    ## logit values

...
sapply(list.files("C:/Users/Ben/Documents/alienR/R",pattern=".R",full.names = T),source)
```
...
#summarize mean, lower 5th and upper 9th
sumframe<-predframe2 %>% group_by(I,J) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95),Traitmatch=unique(Traitmatch))
```
...
```{r}
ggplot(sumframe,aes(x=Traitmatch,y=mean,ymin=lower,ymax=upper)) + geom_line() + geom_ribbon(alpha=0.7,fill="black") + labs(y="Probability of Interaction") + theme_bw()
```

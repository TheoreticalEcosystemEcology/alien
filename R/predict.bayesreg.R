...
predict.bayesreg<-function(x,newdata){
  ...
  df<-parsm1 %>% filter(par %in% c("alpha_mu","beta_mu")) %>% select(Draw,Chain,par,estimate) %>% dcast(.,Draw + Chain ~ par,value.var="estimate") %>% group_by(Draw,Chain)%>%  do((predfun(alpha=.$alpha_mu,beta=.$beta_mu,newdata))) %>% group_by(I,J) %>% summarize(mean=mean(value),lower=quantile(value,0.95),higher=quantile(value,0.05))  

...
#   expect_error(as.alienData(df_ex1, df_int0), "!any(table(dfSpecies$idNodes) > 1) is not TRUE", fixed = TRUE)
#   expect_error(as.alienData(df_ex2[,-1], df_int0), '"idSp" %in% names(dfSpecies) is not TRUE', fixed = TRUE)
#   expect_error(as.alienData(df_ex3, df_int0), "!any(dfSpecies$idNodes %in% dfSpecies$idSp) is not TRUE", fixed = TRUE)
...
#   expect_error(as.alienData(df_ex0, df_int2), "all(dfInteract$idTo %in% dfSpecies$idNodes) is not TRUE", fixed=TRUE)
# })
#
...
#   expect_error(as.alienData(df_ex0, df_int0, dfSite = df_sit0, dfOcc = df_occ2), 'idSp" %in% names(dfOcc) | "idNodes" %in% names(dfOcc) is not TRUE', fixed = TRUE)
...
#   expect_error(as.alienData(df_ex0, df_int0, dfSite = df_sit0, dfOcc = df_occ4), "all(dfOcc$idNodes %in% dfSpecies$idNodes) is not TRUE", fixed = TRUE)
# })
#
#
